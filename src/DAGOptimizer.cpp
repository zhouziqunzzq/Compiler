...
void DAGOptimizer::print() {
    for (auto &node : nodes) {
        printf("#########################\n");
        printf("id: %d\n", node.id);
        printf("priTag: %s\n", st->getValue(node.priTag).name.c_str());
        printf("secTag(%d): ", static_cast<int>(node.secTag.size()));
        for (int &iit : node.secTag) {
            ...
            case DIV:
                if (st->getValue(it->opr1).cat == C &&
                    ...
                    for (auto nit = nodes.rbegin(); nit != nodes.rend(); ++nit) {
                        ...
                        if (nit->op == it->op) {
                            // Non-switchable
                            f1 = find(vCache[it->opr1].begin(), vCache[it->opr1].end(), nit->pl) !=
                                 vCache[it->opr1].end();
                            f2 = find(vCache[it->opr2].begin(), vCache[it->opr2].end(), nit->pr) !=
                                 vCache[it->opr2].end();
                            // Switchable
                            f3 = find(vCache[it->opr1].begin(), vCache[it->opr1].end(), nit->pr) !=
                                 vCache[it->opr1].end();
                            f4 = find(vCache[it->opr2].begin(), vCache[it->opr2].end(), nit->pl) !=
                                 vCache[it->opr2].end();
...
QuadrupleTable DAGOptimizer::optimize() {
    ...
    for (auto &node : nodes) {
        if (node.op != BLANK) {
            q.op = node.op;
            q.opr1 = nodes[node.pl].priTag;
            q.opr2 = nodes[node.pr].priTag;
            q.rst = node.priTag;
            optimizeSymbolTable(q);
            new_qt.push_back(q);
        }
        if (st->getValue(node.priTag).addr != -1 &&
            (st->getValue(node.priTag).cat == V ||
             st->getValue(node.priTag).cat == C) &&
            !node.secTag.empty()) {
            for (auto iit = node.secTag.begin(); iit != node.secTag.end(); ++iit) {
...
void DAGOptimizer::enableTemp(vector<SymbolTableRecord> &t, int id) {
    // TODO: verify that it's safe to do nothing if id is invalid.
    if (id < 0 || static_cast<size_t>(id) >= t.size()) {
        return;
    }
